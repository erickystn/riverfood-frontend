...
import { X, Clock, Package, Moped, CheckCircle, MapPin, CreditCard, ChatText, Receipt, UserFocus } from '@phosphor-icons/react';
import { toast } from 'react-toastify';
import { api } from '../services/api';
import type { Pedido as PedidoBase } from '../pages/MeusPedidos';

// Estendemos a tipagem para garantir que o modal saiba ler o entregador
interface Pedido extends PedidoBase {
  entregador?: {
    id: number;
    nome: string;
    usuario: string;
    ...
    <div className="fixed inset-0 z-[100] flex items-center justify-end">
      ...
      <div className="relative w-full max-w-lg h-full bg-surface-bg shadow-2xl flex flex-col animate-slide-right">
        ...
        <div className="flex-1 overflow-y-auto p-8 space-y-10 custom-scrollbar">
          ...
          {/* DADOS DO RESTAURANTE E ENTREGADOR */}
          <div className="space-y-4">
            <div className="flex items-start gap-4">
              <div className="p-3 bg-river-light/30 rounded-2xl text-river-green"><MapPin size={24} weight="fill" /></div>
              <div>
            ...
            {/* 💡 O ELO PERDIDO: Card do Entregador */}
            {pedido.entregador && (
              <div className="flex items-start gap-4 p-4 bg-blue-50 border border-blue-100 rounded-2xl">
                <div className="w-12 h-12 rounded-full overflow-hidden bg-blue-200 border-2 border-white shrink-0">
                  {pedido.entregador.foto ? (
                    <img src={pedido.entregador.foto} alt={pedido.entregador.nome} className="w-full h-full object-cover" />
                  ) : (
                    <div className="w-full h-full flex items-center justify-center text-blue-600"><UserFocus size={24} weight="fill"/></div>
                  )}
                </div>
                <div>
                  <h4 className="font-black text-blue-900 flex items-center gap-2">
                    {pedido.entregador.nome} <Moped size={16} weight="fill" className="text-blue-500"/>
                  </h4>
                  <p className="text-xs font-bold text-blue-600/70 uppercase tracking-widest mt-0.5">Seu Entregador</p>
                </div>
              </div>
            )}

            <div className="flex items-start gap-4 pt-2">
              <div className="p-3 bg-river-light/30 rounded-2xl text-river-green"><CreditCard size={24} weight="fill" /></div>
              <div>
          ...
          {pedido.status === 'saiu_para_entrega' && (
            <div className="bg-river-dark p-6 rounded-3xl text-center shadow-xl">
              <p className="text-river-green text-xs font-black uppercase tracking-[0.2em] mb-2">Código de Entrega</p>
              <h5 className="text-4xl font-black text-white tracking-[0.4em]">{pedido.codigoEntrega}</h5>
              <p className="text-white/40 text-[10px] mt-4 leading-tight">Confirme este código com o entregador.</p>
            </div>
          )}
        </div>

        <footer className="p-6 bg-slate-50 flex flex-col gap-3 shrink-0">
          {pedido.status === 'pendente' && (
            <button onClick={handleCancelOrder} className="w-full bg-white border-2 border-score-E text-score-E font-black py-4 rounded-2xl hover:bg-score-E hover:text-white transition-all text-sm uppercase tracking-widest flex items-center justify-center gap-2">
              <X size={20} weight="bold" /> Cancelar Pedido
            </button>
          )}
        </footer>
      </div>
